...
router.post("/createDonor", authenticateToken, async (req, res) => {
  try {
    ...
    if (donor_blood_type < 1 || donor_blood_type > 8) {
      return res
        .status(400)
        .send(`Validation Error: Blood type must be between 1 and 8.`);
    }
...
router.put("/updateDonor/:donor_id", authenticateToken, async (req, res) => {
  try {
    ...
    if (rowsUpdated === 0) {
      return res
        .status(400)
        .json({ error: "No changes were made to the donor data." });
    }
...
router.post("/addBloodToBank", authenticateToken, async (req, res) => {
  try {
    ...
    if (checkDonationDate) {
      ...
      res
        .status(200)
        .send({ message: "New blood record created successfully." });
    }
  } catch (error) {
    console.error("Error:", error);
    return res.status(500).send(error.message);
...
router.post("/requestBlood", async (req, res) => {
  try {
    ...
    let nearBlood = branchIn50km.find((item) => {
      if (
        nearDistrictIds &&
        item.dataValues.donor.dataValues.donor_blood_type == blood_type
      ) {
        return item;
      }
    });
    if (nearBlood) {
      ...
      return res
        .status(200)
        .send({ message: "Blood request completed successfully." });
    } else {
      ...
      const isRequestInQueue = await redisClient.lrange(
        `bloodRequestQueue_${blood_type}`,
        0,
        -1
      );
      const existingRequest = isRequestInQueue.find((request) => {
        ...
        return (
          ...
          JSON.stringify(parsedRequest.near50km) ===
            JSON.stringify(nearDistrictIds)
        );
      });

      if (existingRequest) {
        return res.status(400).send({
          message: `Blood request with the same parameters is already in the queue for blood type ${
            bloodTypes[blood_type - 1]
          }. Searching blood every day. Please be patient. STAY SAFE 😷:)`,
        });
      }
      ...
      return res.status(200).send({
        message: `Blood request added to Redis queue for blood type ${
          bloodTypes[blood_type - 1]
        }. It will automatically be processed when blood is available.`,
      });
    }
  } catch (error) {
    console.log(error);
  ...
  try {
	...
	if( today > last_date) {
        ...
        const foundBlood = availableBlood.find((bank) => {
          return (
            parsedRequest.near50km.includes(
              bank.dataValues.branch.branch_district
            ) &&
        ...
        console.log("istenen", parsedRequest.units);
        console.log("düşen", decreasedUnit);
        console.log("bulunan", foundBlood.dataValues.units);
